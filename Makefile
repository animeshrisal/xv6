...
KERNELDEPS = hardware.h riscv.h types.h display.h plic.h proc.h talloc.h tprintf.h trap.h uart.h virtio.h kerneldef.h kernel.h param.h syscall.h gpu_driver.h spinlock.h 
KERNELOBJS = boot.o kernel.o ex.o setup.o display.o plic.o proc.o syscall.o talloc.o tprintf.o trap.o uart.o gpu_driver.o spinlock.o
...
run: user_collision.bin user_rng.bin user_print.bin kernel 
	qemu-system-riscv64 -machine virt -device virtio-gpu-device -smp 2 -bios none -kernel ./kernel -global virtio-mmio.force-legacy=false -device loader,addr=0x81000000,file=user_collision.bin  -device loader,addr=0x82000000,file=user_rng.bin -nographic
